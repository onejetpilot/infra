...
Test-Step '4-5. HDFS и replication=2' { 'smoke' | docker compose exec -T namenode bash -c 'hdfs dfs -mkdir -p /user/m.razhin/.smoke; hdfs dfs -setfacl -m user:hive:rwx /user/m.razhin/.smoke; hdfs dfs -put -f - /user/m.razhin/.smoke/file; hdfs dfs -setrep -w 2 /user/m.razhin/.smoke/file' }
...
Test-Step '8-10. HiveServer2/DDL' { docker compose exec -T hiveserver2 beeline -u jdbc:hive2://localhost:10000/default -n m.razhin --silent=true -e "CREATE DATABASE IF NOT EXISTS smoke_db; CREATE EXTERNAL TABLE IF NOT EXISTS smoke_db.t(id INT) STORED AS PARQUET LOCATION '/user/m.razhin/.smoke/table';" }
